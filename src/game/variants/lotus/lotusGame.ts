...
export function useLotusGame({
  ...
  const startGame = (
    ...
    startOptions?: Parameters<typeof openingTimeline.start>[1] & GameStartOptions & { waitForOpeningReady?: () => Promise<void> },
  ) => {
    animeFixedTts?.reset()
    followDealer.reset()
    ...
    // vibehub 侧还要透传 waitForOpeningReady（联机/单机都靠它等开局就绪）。
    return openingTimeline.start(mode, {
      ...startOptions,
      waitForOpeningReady: startOptions?.waitForOpeningReady ?? waitForOpeningReady,
    })
  }

...
export function bloodFlowSeatView(engine: BloodFlowEngine, seat: Seat) {
  ...
  return {
    ...
    ownActions: window && !engine.paused && !engine.interrupted && !window.decisions[seat] ? structuredClone(window.options[seat]) : [],
    ownScore: window && !engine.paused && !engine.interrupted && !window.decisions[seat] ? engine.currentScore(seat) : null,

...
function snapshot(round = 1, phase: ServerSnapshot['phase'] = 'opening'): ServerSnapshot {
  return {
    kind: 'state_snapshot', roomId: 'ROOM', mode: 'east', phase, round, dealer: 0, honba: 0,
...
describe('openingGate', () => {
  ...
  it('accepts only the first opening snapshot for the active round', async () => {
    ...
    expect(gate.capture(snapshot(2, 'playing'))).toBe(false)
    expect(gate.capture(snapshot(2))).toBe(true)
    expect(gate.capture(snapshot(2))).toBe(false)
    await expect(waiting).resolves.toMatchObject({ round: 2, phase: 'opening' })
  })

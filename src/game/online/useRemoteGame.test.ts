...
describe('useRemoteGame 座位旋转与快照应用', () => {
  ...
  it('把持久深思状态台词按本地座位转交给牌桌', async () => {
    ...
    await connectGame({ onLlmStatus })
    mockSocket!.receive({ kind: 'llm_status', seat: 1, active: true, text: '让我想想怎么打。' })
    expect(onLlmStatus).toHaveBeenCalledWith(3, true, '让我想想怎么打。')
  })
...
describe('useRemoteGame 结算展示与延迟队列', () => {
  ...
  it('结算展示期间到达的下一局快照被延迟；点继续后对话框保留，round_start 到达才开下一局', async () => {
    ...
    await vi.advanceTimersByTimeAsync(1250 + 1500 + 650)
    expect(game.round.value).toBe(2)

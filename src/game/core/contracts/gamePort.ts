...
export interface GameStartOptions {
  ...
  /** 开局动画结束后、第一手开始前暂停（单机联调/夹具用）。 */
  waitForOpeningReady?: () => Promise<void>
  /**
   * **固定牌墙**（单机）。给了就用它开局，不再随机洗牌。
   *
   * 两个用途，都不是"调试后门"：
   * 1. 赛后复现的重跑（分析区 P1 §6）：重跑必须喂**记录里的那一副**环状牌墙，
   *    否则每次重跑都是另一副牌，结束分数自然对不上；
   * 2. 硬护栏用例（"同一副牌在分析开/关两种设置下结果逐项相同"）需要两次开局用同一副牌。
   */
  initialWall?: TileType[]
  /** **固定开局骰子**（两粒）：定时器压缩 / 重跑复现用；不给则随机。 */
  openingDice?: [number, number]
  /**
   * **当局庄家**（分析区 P1 §6 的确定性子局重跑）：不给则沿用引擎当前状态（新的一场是 0）。
   *
   * 为什么重跑必须能给：庄家决定**发牌起点**与结算的庄闲关系。第 2 局以后沿用 0 会让手牌
   * 从一开始就是另一副牌（实测：记录里 13 张的那一家，重跑发到 14 张）。
   */
  dealer?: number
  /**
   * **当局开局分数**（四家，分析区 P1 §6）：不给则从 `resetLocalPlayers` 的缺省分起步。
   * 缺了它，第 2 局以后的结束分数永远与记录对不上 —— 而那不是"复现失败"，是"起点不同"。
   */

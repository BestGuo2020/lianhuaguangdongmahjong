...
const props = defineProps<{ open: boolean; state: BloodFlowPublicState; players: GamePlayer[]; localSeat?: number; filterSeat?: number | null; matchFinished?: boolean }>()
...
<template>
  <Teleport to="body">
    <div v-if="open" class="blood-flow-ledger-backdrop" @click.self="$emit('close')">
...
.blood-flow-ledger { width: min(700px, 96vw); max-height: 92dvh; display: flex; flex-direction: column; border: 1px solid #8a947c; border-radius: 16px; background: #142424; color: #fff2d9; box-shadow: 0 20px 80px #0008; }

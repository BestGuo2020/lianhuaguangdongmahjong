...
const titleStyle=computed(()=>{
  ...
  const left=stage.value==='seat'?[50,77,50,23][relative]:50,top=props.compact?10:stage.value==='seat'?[53,28,12,28][relative]:14
  const unit=stage.value==='main'?(props.compact?52:110):props.cue.tier>=2?(props.compact?36:66):(props.compact?30:54)
  return {left:`${left}%`,top:`${top}%`,width:`min(${(props.cue.title.length*.92+.4)*unit}px,${stage.value==='main'?65:34}vw)`,fontFamily:profile.value.font,opacity:m.opacity,transform:`translate(-50%,${m.y}px) perspective(650px) rotateY(${m.tilt}deg) rotateX(${m.tilt*.3}deg) scale(${1+(m.scale-1)*.55}) rotate(${m.rotation}deg)`}
})
const portraitProgress=computed(()=>props.cue?Math.min(1,Math.max(0,(props.now-props.cue.startedAt-70)/(props.cue.phaseMarks.impact-210))):0)
...
<template>
  <div class="blood-flow-presentation" :data-theme="themeName" :class="{compact}" aria-live="polite">
    <div v-if="cue" :key="cue.id" class="blood-flow-cue" :class="[`tier-${cue.tier}`,`stage-${stage}`,{brief:cue.compact}]" :data-cue-id="cue.id" :data-cue-start="cue.startedAt" :data-phase="phase">
      ...
      <template v-if="themeName==='llmAnime'&&portraitProgress>0&&portraitProgress<1">
        <AnimeActionCue v-for="item in cue.seats" :key="item.seat" :event="animeEvent(item.seat)" :player="players[(item.seat-localSeat+4)%4]"
          :position="['bottom','right','top','left'][(item.seat-localSeat+4)%4]" :progress="portraitProgress" />
      </template>
      <div v-if="cue.kind==='win'&&(phase==='focus'||phase==='impact'||phase==='readable')" class="blood-flow-central" :style="titleStyle">
        <BloodFlowImpactTitle :text="cue.title" :theme="themeName" />
      </div>
...
.winner-0 { left:50%; top:55%; }.winner-1 { left:77%; top:32%; }.winner-2 { left:50%; top:12%; }.winner-3 { left:23%; top:32%; }
...
.feedback-0 { top:60%; left:45%; }.feedback-1 { top:38%; right:14%; }.feedback-2 { top:12%; left:45%; }.feedback-3 { top:38%; left:14%; }
...
.compact .winner-0 { top:53%; }.compact .winner-1 { left:79%; top:29%; }.compact .winner-2 { top:8%; }.compact .winner-3 { left:21%; top:29%; }
.compact .blood-flow-seat-feedback { padding:3px 7px; }
.compact .blood-flow-seat-feedback b { font-size:20px; }
.compact .feedback-0 { top:55%; }.compact .feedback-2 { top:10%; }
</style>

...
复用原 winEffectPresenter 的光束、星芒和粒子，每个赢家定位到对应新牌的楼层；不重复添加实体牌，不新增声音，不修改权威动作节拍。3D 装饰至少保留 750ms 以看清入口渐显，高番沿用队列的 1400/1600ms；中央文字队列仍独立。多响各席独立，资源不被下一次牌桌实例重建清除，结束/恢复/卸载时清理，历史快照不补播。

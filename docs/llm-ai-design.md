...
Kimi K2.5/K2.6 普通局面显式关闭思考，疑难局面显式开启；经中转返回推理字段时，只要最终 `content` 有效就继续解析。Kimi K2 基础版与 Moonshot v1 旧型号按非思考模型调用，未列入能力矩阵的旧型号不擅自追加思考参数。Kimi K3 与完整 GLM-5.3 标记为始终思考模型：普通局面仍调用模型并使用低强度，疑难局面取得座位/整场额度后升级强度；Kimi K3 为 `low → high`，完整 GLM-5.3 为 `low → medium`，并使用 `max_tokens: 512`。GLM-5.3-Flash 在 OrcaRouter 上不使用其不支持的 `none`：普通局面 `reasoning_effort: low` + `max_tokens: 64`，允许简短推理流；疑难局面升级 `medium` + `max_tokens: 512`。Claude Sonnet 5 默认开启自适应思考，快速路径必须显式发送 `thinking: {type:"disabled"}`；条件深思发送 adaptive + summarized display，并移除非默认 `temperature/top_p`。千问结构化输出启用 `response_format: {type:"json_object"}`。若其他供应商返回 `finish_reason=length`、无文本内容或在强制非思考后仍返回 `reasoning_content/reasoning_tokens`，立即回退启发式，不进入普通语义重试。

...
`message` 不属于 `CanonicalAction`。解析成功后，控制器可通过可选的 `onLlmMessage({ playerIndex, text, requestId })` 事件展示；事件失败、被截断或为空都不能影响动作执行。v1 不要求把吐槽写入回放、结算或持久化记录。

### 7.5 发言限流与播放优先级

模型返回 `message` 不等于一定播报。单机、VibeHub 房主浏览器和 Python 联机房间使用同一确定性准入规则：普通吐槽全桌至少间隔 4 秒，并按性格设置座位冷却（话痨 6 秒、激进 8 秒、稳健 12 秒、高冷 16 秒）；吃、碰、杠等关键动作及自摸/吃胡/抢杠胡使用 `important` 优先级并始终放行。展示与合成前只保留第一句且最多 16 个 Unicode code point。

播放层不得积压普通吐槽：已有语音播放时，新普通语音直接丢弃；重要语音可打断普通语音。普通待播项超过 3 秒、重要待播项超过 10 秒视为过期。限流必须发生在 TTS 合成之前，避免为最终不会播放的旧局面浪费服务额度。

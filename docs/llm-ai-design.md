...
每小局胡牌成立后立即启动赛后感言队列，赢家感言不等待胡牌特效或亮牌结束；荒庄则在流局成立后启动。所有启用语音人格的 AI 必须依次发表感言：AI 赢家先说，其余 AI 从赢家起顺时针说失败感言；真人赢家不说话，AI 败者仍依次发言；荒庄时 AI 按座位顺序发表荒庄感言。感言文本由各座位当前模型并行走快速非思考请求生成（硬限制 5 秒），只提供公开输赢结果、性格和表达角度，不提供暗手、牌河或副露；空句、幕后词、超时、网络或解析失败立即回退按性格轮换的保底台词，重复生成句也回退为不同保底句。文本生成并行，播报严格串行；胡牌特效和亮牌可与感言播放并行，但结算窗口必须等待全部感言完成。单机等待每句完整 TTS 播放结束（静音/失败时保留最短文字展示时间）再轮到下一位；联机由服务端串行广播感言并暂缓 settled 快照，全部完成后才放行结算。真人座位永不进入感言队列。莲花广麻与莲花麻将分别通过各自结算适配器接入同一队列规则。

...
快速路径默认使用 `temperature: 0.4`、`max_tokens: 64`、`top_p: 1`、`stream: false`、`n: 1`；供应商能力矩阵可覆盖其强制采样参数（如 Kimi K2.5/K2.6 非思考模式的 `temperature: 0.6`、`top_p: 0.95`）。GLM-5.3/5.3-Flash 始终思考，固定使用 `thinking: {type:"enabled"}`、`reasoning_effort: "low"` 与 `max_tokens: 512`，允许响应带 `reasoning_content`，但最终仍只解析 `content` 的 JSON。千问结构化输出启用 `response_format: {type:"json_object"}`。若其他供应商返回 `finish_reason=length`、无文本内容或在强制非思考后仍返回 `reasoning_content/reasoning_tokens`，立即回退启发式，不进入普通语义重试。
...
| 常用模板 | — | DeepSeek / Kimi / 通义千问 / 豆包 / MiniMax / OpenAI(GPT) / 智谱 / GLM 5.3 Flash (OrcaRouter) / 自定义（Base URL + 示例模型） |
...
| 温度 / max_tokens | 0.4 / 64（GLM-5.3/5.3-Flash 为 512） |

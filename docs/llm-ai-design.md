...
发言频率只由程序控制，不再要求模型按性格自行省略 `message`。单机、VibeHub 房主浏览器和 Python 联机房间使用同一确定性准入规则：普通台词全桌至少间隔 2 秒，并按性格设置座位冷却（话痨 3 秒、激进 5 秒、稳健 7 秒、高冷 10 秒）；吃、碰、杠等关键动作及自摸/吃胡/抢杠胡使用 `important` 优先级并始终放行。展示与合成前先过滤“引擎、候选、编号、模型、系统、提示词、AI、基线”等幕后词汇；命中则整句丢弃且不占用冷却。通过过滤的台词只保留第一句且最多 16 个 Unicode code point。

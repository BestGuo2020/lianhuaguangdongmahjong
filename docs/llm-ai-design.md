...
- `opponentRisk`（2026-09-10 追加，2026-09-12 扩到 v2 门清读牌与 v3 公开番型，前后端同源）：档位由公共信号取最大值 —— tier1 `低`＝半染手 / 短牌河 / 门清「牌河中张密集」（七对弱信号）/ 门清「牌河少打某花色」（短牌河兜底），tier2 `中`＝染手嫌疑（副露 ≥2 组且同花色占比 ≥0.75）/ 副露 ≥3 组 / 箭牌或风牌成组 ≥2 / 已锁手且已胡过 / 门清「牌河无字牌幺九」（十三幺·字一色）/ 门清「牌河几乎未打某花色」（九莲·清一色），tier3 `高`＝三组箭牌或三组风牌 / 门清「牌河零字牌幺九」/ 门清「某花色一张没打」。`payment = 40 × 公开张数档位(0.25/0.1/0) × max_对手(档位倍率 × 花色系数 × 中张系数)`，档位倍率 4/16/32、花色系数 1/0.5、十三幺·字一色轴上中张系数 0.25（嫌疑花色内的中张不打折）；一次弃牌最多被一家胡，所以取最高权重的对手而不相加；已锁手家不吃现物折扣与花色折扣。**多现 ≠ 安全**：十三幺·字一色轴上的字牌与幺九，公开张数档位保留下限 0.1（该牌型每种只要一张）。全部 tier=0 或开关 `'off'` 时必须退化为旧的 `40 × 公开张数档位`（逐位一致，有回归测试）。读牌永远是概率性的：门清大牌能读到 tier2/3，但判错与"读到也防不住"都在预期内，不得在文案里宣称必防。
- **v3 公开番型与兜/弃政策（2026-09-12）**：威胁档取 `max(牌河读牌, 已公开番型)`；番型倍率 ≥4/≥8/≥16 → tier 下限 1/2/3，弱番不设档。按番型定危险轴（`axisSource='known'` 对锁手家同样成立；`'inferred'` 对锁手家禁用）：十三幺/字一色/清幺九/混幺九 → 字牌幺九轴；清一色/九莲/绿一色 → 花色轴（**哪一门取自公共批次 `source.tile`**）；混一色 → 字牌算本门；大三元/小四喜/字一色 → 字牌刻子轴。血流 prompt 追加 `opponentPatterns`（每座位已公开番型 label+倍率）与 `defense{mode,reasons,ownCanTenpai,ownBestWait,ownAnyWaitReachable,ownCeiling,restricted}`；兜牌（未听牌且对手十六倍级）时**候选层收窄**——吃碰杠候选不出现、弃牌只剩最小赔付档，`defense.restricted=true`；两个出口（打一张即精吊任意听、我方上限不低于对手）不受限。规矩：胡永远保留，绝不放过能胡的牌。
- **候选层守卫必须两边一致（2026-09-12 修）**：血流 LLM 候选曾直接照搬 `bloodFlowAiActions` 的全部合法动作，缺了非血流 `claimCandidates` 里的 `pengWouldDiscardClaimedTile` 守卫，导致"能大明杠却碰牌并打掉该牌"。现血流在候选层加 `dropDominatedPeng`（能大明杠时不给"碰"），引擎与 LLM 共用同一份候选，行为与本地 AI 的"能杠必杠"一致。**规则**：凡"本地 AI 有的候选层取舍"，LLM 路径必须复用同一实现，不得各写一份。
- **真·大牌路线（v4，2026-09-12）**：同一手"13 种幺九 + 闲牌 + 有小胡"，`deepseek-v4-pro` 会放弃小胡做十三幺，而 glm-5.3 / glm-5.3-flash / gpt-5.6-luna / gpt-5.6-sol **全部直接胡** ⇒ 大牌路线不能靠提示词劝说。改为**引擎定方向、模型只在路线内选怎么打**：路线判定（十三幺 ≥12 种幺九；九莲一门 1-9 齐且 ≥12 张）+ 时机门槛（牌墙 ≥20 或落后 ≥300 分）成立时，在 LLM 候选层撤掉"胡"（路线收益 320 点 ≥ 立即胡 ×2）、撤掉吃碰杠、弃牌只留不掉路线的牌，prompt 增加 `bigHandRoute{label,progress,need,committed}`。生效范围仅 LLM 座位（`BLOOD_FLOW_LLM_AI`），普通 AI 一行不变。实测（10 局）：触发率 21.9%→**1.6%**、十三幺 **0→6 次**、总胡次数与七对/混一色**回到基线**。
...
自定义中转中的 Kimi K2.5/K2.6 使用独立思考预算：非思考路径保持 `thinking.disabled + 64`；条件思考路径使用 `thinking.enabled + 2048 + response_format:json_object`。官方 Moonshot K2.5/K2.6 不套用该中转扩容，维持官方请求参数与 512 上限。

OrcaRouter 域名下的 DeepSeek、Qwen 与 Kimi 条件深思请求统一使用 `max_tokens: 65536`，避免中转层把 reasoning 与最终 content 共用较小额度而截断。该规则不作用于普通路径、官方 API、其他未知中转或其他模型供应商；牌桌请求超时仍可先于 token 上限终止请求。

条件深思预算按“端点 + 模型 + 思考档位”在当前运行会话内自适应：首请求沿用供应商初始值，之后使用最近最多 32 次样本的 P99 reasoning token + 96 个最终 JSON 预留并向上取整；发生 `length` 时下一次预算至少翻倍，最大 65536。若同一键在 65536 上限连续截断 2 次，则临时停用该模型条件深思 30 分钟，always-on 模型仍走普通低强度，其他模型走快速模式；不在同一局面即时扩容重试。

条件深思默认开启：候选评分差不超过 8 且候选特征确有差异、牌墙不超过 12、对手威胁达到 70、预期分差影响达到 800，或命中 2% 审计抽样时，可切换到供应商思考参数。庄家首打及当前 AI 前两巡不因“候选接近”或审计抽样升级思考，早巡对手公开威胁需达到 90；AI 自身出现多个不同听牌方案，或碰/杠可能破坏听牌时仍作为强触发。完全同质的零分差候选交给确定性引擎，不消耗深思额度。每个AI座位每小局最多 2 次、其中普通软触发最多 1 次，为听牌、残局等强触发预留机会；全桌整场最多 24 次。对手暗手和用户私有听牌提示不进入 LLM 快照，仅按牌河与副露估计公开威胁。可关闭思考的模型在未触发时走快速模式；`always-on` 模型始终调用，额度只限制从低强度升级，不限制普通低强度请求。统计分为“思考请求”（包括 always-on 低强度和实际返回推理流）与“升级请求”（命中触发器并消耗额度）。`always-on` 的普通 low 请求不先展示“让我想想怎么打”等思考台词，也不走该台词的 TTS；收到流式推理块后仍正常展示安全进度气泡。默认模型请求硬截止 40000ms；预置关闭 `timeoutEnabled` 后牌桌请求不设置定时中止。仅自摸/抢杠胡玩法不使用“对手防铳威胁”触发器。已开启、升级或始终思考的模型返回推理块时，按块序号生成“观察公开牌局 / 整理规则约束 / 比较可行动作 / 评估攻守节奏 / 复核选择”等客户端安全进度；生成过程不接触暗手、候选详情或供应商原始推理。每个进度脉冲直接替换气泡中的当前一句，不累积历史、不裁切、不滚动，按普通气泡宽度完整展示。等待期间按性格轮换的状态短句可走 TTS；安全进度与原始推理均不进入普通台词历史、日志、发言限流或 TTS，状态气泡在模型返回或超时前不自动消失。

每小局胡牌成立后立即启动赛后感言队列，赢家感言不等待胡牌特效或亮牌结束；荒庄则在流局成立后启动。所有启用语音人格的 AI 必须依次发表感言：AI 赢家先说，其余 AI 从赢家起顺时针说失败感言；真人赢家不说话，AI 败者仍依次发言；荒庄时 AI 按座位顺序发表荒庄感言。感言只使用按性格与输赢类型轮换的本地默认台词，不额外请求大模型；同局同性格 AI 按队列位置错开句子，跨局继续轮换。播报严格串行；胡牌特效和亮牌可与感言播放并行，但结算窗口必须等待全部感言完成。单机等待每句完整 TTS 播放结束（静音/失败时保留最短文字展示时间）再轮到下一位；联机由服务端串行广播感言并暂缓 settled 快照，全部完成后才放行结算。真人座位永不进入感言队列。莲花广麻与莲花麻将分别通过各自结算适配器接入同一队列规则。

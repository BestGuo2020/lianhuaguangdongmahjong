...
### 12.2 线上验收发现并修掉的四个真实缺陷

这四个都**无法用本地 mock 复现**（`mockVibeHub` 进程内投递、无大小上限），只能上线跑出来：
...
4. **经典场次收尾挂错了数据流**（第二轮，加经典场断言后跑出来）：房主自视快照走 `snapshotReconciler.apply()`，**不经过 `handleMessage`**，所以挂在 `handleMessage` 上的收尾永远等不到 `matchFinished` ⇒ 取证里"两侧各 4~5 局、steps 齐全且一致、结算帧四家明牌，但 `matches: []`"（只有局、没有场）。改为挂在 `onLocalSnapshot` 上。
   同一轮还修掉一个**既有验收用例缺陷**：血流放行到 P2P 后，规则列表里出现两个「莲花麻将」开头的条目，`Phase 11V` 用的 `/^莲花麻将/` 严格定位器同时命中两个而直接失败（与回放无关，但挡住了整条经典场验收）。
...
第二轮（2026-09-16）把同一套断言挂到**经典场**（`Phase 11V 线上两账号完成莲花麻将完整东风场`）并重跑：经典场 + 血流普通机器人场 **`2 passed`（约 11 分钟）**。经典场取证同上：两侧各 5 局、逐局 steps 一致、结算帧四家明牌、座位 0/1、位次 3/4。这轮跑出来的两个缺陷见 §12.2 第 4 条。

第三轮（2026-09-16）新增**补局场景**（`线上回放：客机中途丢失本地牌谱后，场末仍能自动补齐为完整全知牌谱`）：
开局后清空客机本地回放库（模拟"中途重进/换设备，此前那些局本机根本没有"），打到终局断言它自动补齐。
**`1 passed`（4.9 分钟）**：`[REPLAY-ONLINE] replay-gap-fill 联机牌谱：房主 4 局 / 客机 4 局（均为全知）；座位 0/1 位次 3/4`。
注意造法：**单纯刷新页面不会丢牌谱**（已在 IndexedDB 里），必须清库才是确定性的"缺局"。

### 12.4 补局升级为「问任意持有者」（2026-09-16 第三轮）

原来的补局只有房主能应答 ⇒ 房主清过库/换过权威时，缺局方永远补不齐。现在：

| 机制 | 做法 |
|---|---|
| 持有清单 `replay_inventory` | 落库后广播"本机这场有哪几局、有没有场次记录"，缺局方据此知道向谁要（不依赖房主） |
| 补局服务器 `createRemoteReplayServer` | **每个参与者都有**：谁手上留着对方缺的局谁就补；房主中继仍立即应答，其余人错峰 |
| 错峰选举 | 按哈希 `(本机标识, 场次, 局号)` 算延迟，先到者发；共享登记簿一登记，其他人看到清单就取消（只补一份） |
| 场次记录可补 | 场末掉线连场次记录都没收到的玩家会请求 `wantMatch`，任意持有者发回场次记录，再据它补齐缺局 |
| 缺口去重与上限 | 请求按"缺口内容"去重（同样的缺口刚要过就不再要），定期重试才计次数，有上限、会放弃；`onServeError` 暴露补发失败（此前被静默吞掉） |
| **本地侧也要复核** | 缺局判断不能只挂在"收到牌谱"上：**房主是自己录制、不接收任何东西**的一方，它本地缺失时也必须主动去要（`peer.review(matchId)` 挂在落库回调上）。没有这条，"由非房主补局"只做了一半。 |
...
- **经典（莲花麻将）联机回放也已线上验收通过**：断言抽成共用 helper `expectOnlineReplayPaipu({ pages, testInfo, label, rulesetPattern })`，经典场、血流两场与补局场景共用。经典场取证：两侧各 5 局、**逐局 steps 完全一致**（108/110/32/113…）、结算帧四家明牌、`humanSeat` 0/1 且位次各按自己（3 位 / 4 位）。
  验收账号长期复用、本地库会累积历次牌谱 ⇒ helper 按 `startedAt` 取**最新一场**，玩法选择用精确正则（经典场用 `莲花麻将(?!·)`，因为 `莲花麻将` 也会命中 `莲花麻将·血流`）。
- **「非房主持有者补局」已线上验收**（2026-09-16 第三轮第二条场景）：`线上回放补局：房主本地牌谱丢失后，由客机（非房主持有者）补回缺局` —— 开局后清空**房主**的本地回放库，此前的局只剩客机手上有，场末房主仍拿回完整一场（取证 `replay-holder-serve-*.json`：7 局、逐局 steps 非零、结算帧四家明牌，且与客机侧完全一致）⇒ 缺局确实由**非房主**补发。
  （原计划用"三真人 + 清房主库"验证同一件事，但 `tmp/online_test` 的第 3 个账号 OAuth 在限定会话内始终未完成、无法登录，遂改用两账号的上述造法，判据等价且更省。）
- **master 的 WS 联机**（服务端权威，客户端只有"所见"信息）本次未动，要做全知需后端提供牌谱接口。

...
- **M1**：录制 + IDB + 大厅入口 / 列表 + 3D 回放壳 + 控制条（单步 / 播放 / 局切换）+ 和了与荒庄帧 —— 覆盖 **莲花广麻 + 莲花麻将**
- **M2**：**血流**（worker 旁观视角 + 一局多胡步进）+ 牌谱事件侧栏 + 跳鸣牌节点 + 视角开关（**主题不提供切换**）
- **M3**：删除 / 清空 / 保留策略 UI、导出 JSON 牌谱、从结算页直接「查看本局回放」
...
| 项 | 结果 |
|---|---|
| 单测 | 回放相关 **37 项**全绿（格式/存储/录制器 15 项含 10 种动作映射、广麻整场 3 项、莲花整场 2 项、血流 3 项） |
| 全量回归 | `pnpm test` **143 文件 / 1479 项通过**（新增 20 项），`pnpm typecheck` 通过 |
| 端到端 | `tests/e2e/replay.spec.ts`：真实引擎各打完整场东风场（广麻 / 莲花麻将 / 血流）→ 落库 IndexedDB → 真实 App 大厅列表 → 3D 回放视图 → 单步 / 末步 / 牌谱跳转 / 切局 / 视角开关 / 按记录主题渲染，**1 个用例全绿** |
| 截图 | `work/replay-e2e/*.png`（列表 / 三玩法开局帧 / 结算帧 / 暗牌视角 / 单步） |

...
测试：`remoteRelay.holders.test.ts` **8/8**（房主清库后由他人补齐、同一请求只补一份的两场景、场次记录可补、无人持有按上限放弃、错峰哈希、登记簿抑制与过期）；回放套件**连跑 20 轮 0 失败**；两侧全量 1519 / 1667 项通过。
...
- **「非房主持有者补局」尚未线上验收**：线上门禁是两账号 + 2 机器人，房间里只有房主与那一名客机持有牌谱 ⇒ 要验"房主也没有、由第三名玩家补"，需要**三真人**场景（`tmp/online_test` 有 3 个账号，但需另写三浏览器编排）。该路径目前由 8 项确定性多参与方测试覆盖（含"房主清库"场景），线上只验到了"缺局 → 请求 → 应答 → 落库"这条流水线。

...
## 8. 2026-09-24 单机分析记录修复

- 经典玩法的控制器钩子现在与本座位当前决策窗口关联：实际模型请求写候选、当时推荐、提示词、请求型号、回答和回退。规则短路的胡牌单列 `rule-auto`；真人与普通 AI 的选择直接从引擎合法动作识别，不再依赖模型候选钩子。
- 暗杠回执保留牌码（如 `s9`），不再把显示名「九条」与牌码比较后误报 `state-changed`。
- 配置记录包含可序列化的经典规则和座位模型设置，不含 API Key 或自定义服务地址。展示牌谱的 `analysisRecorded` 取本场本地分析开关，实际质量仍由导出包内容核验。
- 分数变化逐笔入账，局末另记零分 `round-end/*` 标记；旧版「逐笔流水 + 整局净分」相加会重复计分。旧分析包不补造缺失的模型请求，重新导出时 `completeness` 会按已知缺口标为 `partial`，其 `reproductionCapable` 仍可为 true。
- 验收：定向单测、类型检查、生产构建通过；浏览器夹具逐局重放 5/5、命令 303/303，真实 App 分析自动刷盘通过。该段只涉及单机分析路径，联机行为未改。

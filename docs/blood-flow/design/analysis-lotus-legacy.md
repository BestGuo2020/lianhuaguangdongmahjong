...
## 13. 2026-09-24 单机分析记录修复

- 延续原有模型请求接缝：候选、推荐、模板与 133 次真实尝试在样例对局中已可关联。录制器现在也能从引擎合法动作识别无模型请求的真人和规则自动选择；规则短路的胡牌标为 `rule-auto`，普通 AI 标为 `local-strategy`。超时且没有原文时，回答标未知而非「已知空文本」。
- 决策前态新增仅由本座位可见信息计算的内容指纹。过牌回执不再要求手牌张数不变：下一窗口开启前的补摸不代表过牌被改判。
- 计分改为跟庄、各类杠、最终胡牌的逐笔变化，局末写零分 `round-end/*` 标记。流水可连续加到终局分数；旧版每局一条净分汇总仍能读，但重新导出会标出缺少逐笔账本。
- 配置记录包括玩法参数和已生效的座位模型设置，排除密钥与私有服务地址。旧文件里缺失的采样、服务端返回型号和用量不会被事后编造。
- 验收：浏览器单机夹具六项通过，包括 5/5 逐局重放、模型接缝、开关对照和真实 App 落库；全量前端单测 2129 通过、2 跳过，类型检查及生产构建通过。本轮不修改联机层。旧 §12.6 的分支镜像过程是历史记录；当前同步操作以仓库根 `AGENTS.md` 为准。

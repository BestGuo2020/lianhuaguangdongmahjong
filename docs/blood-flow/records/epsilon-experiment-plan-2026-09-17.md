...
| tag | ε（点） | 口径 | 预计调用/局 | 预计调用总数 | 每片上限 `SHARD_MAX_CALLS` | 预计输入 token |
|---|---|---|---|---|---|---|
| `eps0` | **0** | top-two | 27.3 | 8,191 | 2,294 | 3,768 万 |
| `epss` | **10** | top-two | 20.2 | 6,050 | 1,695 | 2,783 万 |
| `epsm` | **40** | top-two | 9.5 | 2,854 | 800 | 1,313 万 |

**合计 ≈ 1.71 亿输入 token + 82 万输出 token ≈ 1.72 亿 token**（口径：4,600 输入 + 50 输出 token/次）。

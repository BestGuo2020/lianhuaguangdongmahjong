...
## 3.5 任务零（**不花钱，建议第一个做**）：量出"能跳过多少窗口"

**目的**：在花任何 API 钱之前，先算出 ε-gating 能省多少调用与多少钱。

**做法**：用 `LLM_SEATS=0`（纯 EV，**零 API 调用** ✓）跑 1000 局，在每个决策窗口记录
**本地 top-2 候选的价值差**（用 `patternPotentialTotal` / `patternPotentialEv` 或 `bloodFlowEvContext` 的可用值 ✓），
输出"价值差分布直方图 + 各阈值下可跳过的窗口占比" ✓。

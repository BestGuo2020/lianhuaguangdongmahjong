...
把其中 1 个座位换成外接大模型（`deepseek-flash` / `deepseek-v4-pro`，**只走官方端点**），
...
| `pro-1seat-300` | pro 1 座 | 300 | −251.5 | −2.58 ✓ | 13.98% |
...
**成本**：2 片 × 200 局 ≈ 30~40 分钟。

## 4. 任务 ε：ε-容忍约束实验（产品代码改动）

**问题**：给模型的自由度划边界后，亏损能否归零？

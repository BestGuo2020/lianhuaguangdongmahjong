...
## 八、融合 pilot 终判（拒绝）与全线结论

**fusion pilot（N=30，seeds 4000–4029，fusion vs baseline）**：配对差 mean **−188**（sd 1969，
CI95 [−897,+514]，18/30 为正）；均值条款（≤−100）触发 → **拒绝**。60/60 包完整、0 请求失败。

诊断（比判定更重要）：fusion 放炮 **191** vs baseline **192**——危险重排**没有减少放炮**，
却因 λ=400 的弃牌重排牺牲了 EV 的进攻收益（meanDelta −489 vs −302）。即：危险信号有判别力
（AUC 0.716）但**边际信息不足**——EV 自带安全档已吃掉大部分危险信号，λ 惩罚只扭曲了弃牌选择。
按预注册不偷调 λ；若未来重启融合需新预注册（新 λ + 新种子段 + 或许改为「仅否决不安档候选」的硬门控）。

**全线结论**：Jev 蒸馏模型作为①独立棋手（E4 −296）与②危险重排顾问（fusion −188）均被预注册
标准拒绝；其剩余已证明资产 = 基础设施（gold 管线/蒸馏/校正/信号检验/复现导出）与校正后的
概率可读性（ECE 0.043）。唯一未试的改进路径 = 蒸馏迭代（head-5000+2 epochs+DAgger 自轨迹补采）；
若该路径也失败，Jev 线在本玩法退役，保留为分析/表演用途。

## 九、蒸馏迭代第二轮（iter2，结案：门槛不过 → Jev 决策线退役）

**预注册协议**（目标 goal-ecfbdef3）：① DAgger 补采 20 场（seeds 6000–6019，seat0=学生）→
dagger-v1.jsonl；② 训练 = head-N(teacher) + dagger，rows=2 + 1 epoch + bf16 + brier 0.5；
速率应急（预注册）：10 分钟稳态 >3.5s/step 裁数据；③ 门槛 dev acc≥0.75 且 NLL≤1.2
（iter1 基线 0.706/1.407）；④ pilot N=30 seeds 4100–4129 vs baseline，老阈值结案；
⑤ 窗口 ≤8h/¥30；⑥ 不过门槛 → 退役报告。

**执行日志**：
- DAgger 补采 ✅ 1922 样本（96/场，7.1min）；
- 排障：serve（16.2GB）与训练争抢 vGPU 切片 = 历史"莫名 OOM"真凶之一；日间争用 9.25s/step
  （夜间 4.8 的 1.9×）→ 速率应急触发 → 二级裁剪 head-3000+dagger=4922；expandable_segments
  峰值 25.5GB（vs 无它 31.4GB 顶死）；
- **排程偏离披露**：原计划 22:55 夜间自动点火（避争用），但用户反馈「sleep 等待浪费资源」——
  实例空闲计费才是真实浪费（sleep 仅耗 agent 轮次、不耗算力/¥）→ 改为**日间缩减数据直跑**：
  head-500(teacher 锚点)+dagger-1922 = **2422**，chunk-rows 2 + expandable_segments，零空闲。
  三级应急（预注册）：10 分钟稳态 >9.5s/step → 再裁 dagger-only 1922；
- 实例两次重启（12:39 用户开机 / ~12:45 AutoDL 换端点 weste:24832→westc:14016）杀掉首次点火，
  新端点 12:49:58 重启训练（数据盘保留，train2-all=2422 ✓）；10 分钟稳态 **7.7s/step**（<9.5，
  三级不触发），显存 28.5GB/32GB，ETA ~5.2h（约 18:10 完成）；
- 提交：2e28742（DAgger）/600d9cd（iter2 脚本）/0b7e0ee（夜间配置）/d71d23b（校准脚本）/
  1e3a47d（日间缩减配置）/8a5cafe（本记录骨架）。

**日间实测（训练完成，结案数字）**：
- 训练：12:49:58→19:31:09（实例时间），历时 **6h41m**，2422 步 1 epoch，均速 **~9.9s/step**
  （稳态 7.7→尾段争用升至 ~12.6），末步 loss 0.595（bf16 噪声大，区间 0.27–3.6）；
  显存峰值 28.5GB/32GB（expandable_segments + rows=2，未触发三级应急）；
- **门槛终判（预注册 ③：dev-v3 acc≥0.75 且 NLL≤1.2）**：
  | 口径 | acc | NLL | ECE |
  |---|---:|---:|---:|
  | 脚本内 eval（T=1，n=858） | 0.734 | 0.9406 | — |
  | **独立 openjev eval 对拍（T=1，n=858）** | **0.7354** | **0.9410** | 0.1449 |
  | 独立 eval + 温度校正 T\*=1.771 | 0.7354（argmax 不变） | 0.7808 | 0.0567 |

  → **acc 0.7354 < 0.75 ✗（差 1.5pp）；NLL 0.941 ≤ 1.2 ✓；AND 门槛 = 不过**。
  独立 eval 与脚本内 eval 逐位吻合，坐实决定性数字（与 iter1 同规格对拍）。
- **iter2 vs iter1（如实画像：全面改善但未越红线）**：
  | dev-v3 指标 | iter1 | iter2 | Δ |
  |---|---:|---:|---:|
  | acc（T=1） | 0.706 | 0.735 | **+2.9pp** |
  | NLL（T=1） | 1.407 | 0.941 | **−0.466** |
  | ECE（T=1） | 0.204 | 0.145 | −0.059 |
  | 校正 T\* | 2.590 | 1.771 | — |
  | 校正后 NLL | 0.912 | 0.781 | −0.131 |
  | 校正后 ECE | 0.043 | 0.057 | +0.014 |
- **pilot：未执行**——门槛不过即未取得 ④ 资格（预注册「才进 ④」），seeds 4100–4129 保持未用；
  按纪律不偷调阈值、不事后跑未预注册的对局。
- 预算实测：迭代计算 ~6.9h（训练 6.7h + eval/校准 0.2h）/ **~¥11**（含开机空闲共 ~7.6h/¥12），
  在 ≤8h/¥30 窗内；GPU 已空闲，建议用户**关机**止血（无 API token，agent 不能远程关机；
  关机保留数据盘，adapter/数据/日志留存可复盘）。

**结案判定（触发预注册 ⑥）**：iter2 蒸馏迭代把 dev 指标全面推向正确方向（acc +2.9pp、
NLL −0.47），但**未越过预注册 acc≥0.75 迭代红线**（0.735，差 1.5pp）。叠加前序证据——
iter1 虽过其宽门（acc≥0.50）却在 E4 对局被拒（配对 −296）、fusion 危险重排亦被拒（−188）——
本轮按 ⑥ **如实报告：增益不足，Jev 决策线退役进分析/表演抽屉**。保留资产 = 全套基础设施
（gold 管线 / 蒸馏 / 温度校正 / 信号检验 / 复现导出）与校正后可读概率（iter2 ECE 0.057、
NLL 0.781，可作分析/表演用途的概率展示）。若未来重启决策线，需**新预注册**（更大数据/多 epoch/
7B 或改门槛），不在本轮授权内。

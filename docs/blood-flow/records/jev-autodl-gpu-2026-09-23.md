...
- 排程：日间不跑（争用浪费预算），22:55 后台自动点火，预计 ~6.6h → ~05:30 训练完；
  空闲待命成本归属常设温待命决定（书面声明，不计迭代窗口）；
- 提交：2e28742（DAgger）/600d9cd（iter2 脚本）/0b7e0ee（夜间配置）/d71d23b（校准脚本）。

**夜间数据回填位**（训练完成后填）：
- 训练：步速 __s/step、总时长 __、loss 末值 __；
- 门槛：dev acc __ / NLL __（≥0.75/≤1.2 过？）→ __；
- 校准：T* __、dev NLL __→__、ECE __→__；
- pilot：配对差 mean __（sd __、CI95 [__,__]、正场 __/30）→ 判定 __；
- 预算实测：迭代计算 __h / ¥__。

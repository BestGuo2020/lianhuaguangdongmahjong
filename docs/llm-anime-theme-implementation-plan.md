...
#### Phase R7：动作立绘放大与一番街式大字（R6.21，2026-09-02 立项）

> 立项确认（2026-09-02）：①三端一起放大（PC 立绘高约 45% 视口高、平板随容器等比、手机约 30% 视口高）；②一番街式大字倾斜特效**取代**现有右下贴边小字；③PC 贴边字先向内收约 4~6px（右/下缘各收回）。

- 立绘放大以 R6.19 座位锚点为基准，通过共享变量（cue 基础尺寸 + `--action-art-scale` 分档）实现，不重写座位几何。
- 硬约束（三端 × 四座位）：放大后的立绘**不压对应牌河**（对家=顶部牌河、上/下家=侧向牌河、本家=底部牌河）、**完整可见不超出屏幕**、不遮挡顶栏按钮与本家手牌。
- 大字特效：动作字改为大号、右上倾斜（约 8~14°）、亮色渐变描边 + 厚度阴影，覆盖在立绘中央偏下、压在角色主体前；`aria-live`/文案与 `--anime-accent` 不变；三端随立绘比例缩放。
- 验收：E2E 新增“放大立绘 × 三端 × 四座位”断言（立绘矩形在视口内、与手牌/顶栏/牌河代理矩形零重叠）；保存修改前后截图与大字视觉截图；typecheck、单测、响应式 E2E、构建、`git diff --check` 全绿。
